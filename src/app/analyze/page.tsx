...
export default function AnalyzePage(){
  ...
  return(<main><Navbar step={2}/>
    <div className="fixed top-[53px] left-0 right-0 h-[2px] bg-border z-[199]"><div className="h-full bg-acid transition-all duration-500" style={{width:`${(step/3)*100}%`}}/></div>
    ...
    <div className="max-w-[740px] mx-auto px-6 pt-8 pb-32">
      {step===1&&(<div><div className="mb-9"><div className="font-mono text-[9px] tracking-[0.2em] uppercase text-acid mb-2">01 of 03 — Your Idea</div><h2 className="font-bebas text-[44px] leading-[0.95] mb-2.5">TELL US YOUR IDEA</h2><p className="font-mono text-[12px] text-muted1 leading-[1.7]">Describe your idea, industry, and target customer.</p></div><IntakeForm/></div>)}
      {step===2&&(<div><div className="mb-9"><div className="font-mono text-[9px] tracking-[0.2em] uppercase text-acid mb-2">02 of 03 — Business Model</div><h2 className="font-bebas text-[44px] leading-[0.95] mb-2.5">WHAT SPACE ARE YOU IN?</h2></div>
      ...
      {step===3&&(<div><div className="mb-9"><div className="font-mono text-[9px] tracking-[0.2em] uppercase text-acid mb-2">03 of 03 — Stage & Resources</div><h2 className="font-bebas text-[44px] leading-[0.95] mb-2.5">WHERE ARE YOU NOW?</h2></div>
    ...
    <div className="fixed bottom-0 left-0 right-0 px-6 py-3.5 z-[190]" style={{background:'linear-gradient(to top,var(--bg) 65%,transparent)'}}>
      <div className="flex gap-2.5 max-w-[740px] mx-auto">
        ...
        <button onClick={step<3?nextStep:handleAnalyze} disabled={loading} className="flex-1 bg-acid text-black border-none font-bebas text-[22px] tracking-[0.1em] cursor-pointer hover:-translate-y-px hover:shadow-[0_8px_32px_rgba(212,255,0,0.3)] disabled:opacity-40 disabled:cursor-not-allowed py-4">
          {loading?'Analyzing...':step<3?`NEXT — ${LABELS[step].toUpperCase()} →`:'ANALYZE MY IDEA →'}
        </button>
      </div>
    </div>
  </main>)
